                        ...
                        <div className="text-right">
                            <div className="flex items-center justify-end space-x-1">
                                {[1, 2, 3, 4, 5].map((star) => (
                                    <Star
                                        key={star}
                                        className={`h-5 w-5 ${star <= Math.round(giver.avg_review_rating)
                ...
                <Dialog open={!!selectedBooking} onOpenChange={() => setSelectedBooking(false)}>
                    <DialogContent className="bg-gray-900 border border-gray-700 text-white max-w-lg">
                        ...
                        <div className="space-y-3 mt-4">
                            ...
                            {/* ✅ NEW DATE FIELDS */}
                            <label className="block text-gray-400 text-sm mt-4">Start Date</label>
                            <input
                                id="giver-booking-start"
                                type="date"
                                className="w-full bg-gray-700 border border-gray-600 rounded-md p-2 text-white"
                            />

                            <label className="block text-gray-400 text-sm mt-4">End Date</label>
                            <input
                                id="giver-booking-end"
                                type="date"
                                className="w-full bg-gray-700 border border-gray-600 rounded-md p-2 text-white"
                            />
                            ...
                            <Button
                                ...
                                onClick={async () => {
                                    const startDate = document.querySelector("#giver-booking-start")?.value;
                                    const endDate = document.querySelector("#giver-booking-end")?.value;
                                    if (!startDate) return toast.error("Please select a start date.");

                                    const success = await createBooking({
                                        giver_id: giver.giver_id,
                                        service_id: giver.service_id || 1,
                                        start_date: startDate,
                                        end_date: endDate || startDate,
                                        total_price_RWF: giver.hourly_rate_RWF || 0,
                                    });

                                    if (success) setSelectedBooking(false);
                                }}
                            >
                                Confirm Booking
                            </Button>
